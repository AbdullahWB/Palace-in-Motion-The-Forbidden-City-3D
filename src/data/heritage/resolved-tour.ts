...
function resolveTourStep(stop: (typeof tourStopDefinitions)[number]): ResolvedTourStep {
  if (stop.focusZoneId) {
...
export const resolvedTourSteps: ResolvedTourStep[] =
  tourStopDefinitions.map(resolveTourStep);

export function getResolvedTourStepById(stepId: string | null | undefined) {
  return resolvedTourSteps.find((step) => step.id === stepId) ?? null;
}
